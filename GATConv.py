...
class GATConv(MessagePassing):
    def __init__(self, in_channels, out_channels, alpha_threshold, reattn, central, self_loops=False):
        ...
        self.fc = torch.nn.Linear(3*out_channels, out_channels)
    ...
    def message(self, edge_index_i, x_i, x_j, size_i, y_i, y_j, z_i, z_j):  #x_j:[564365, 64] 
        self.alpha = torch.mul(x_i, x_j).sum(dim=-1)
        self.alpha = softmax(src=self.alpha, index=edge_index_i, num_nodes=size_i)  #torch.Size([564365])

        tmp = self.alpha.unsqueeze(1).expand_as(x_i)

        if self.central == 'central_item':   
            if self.compare == '>':
                result = torch.where(tmp>self.alpha_threshold, (x_i+y_i+z_i)/3, x_i)
            else:
                result = torch.where(tmp<self.alpha_threshold, (x_i+y_i+z_i)/3, x_i)
            
            
            if self.reattn:
                self.alpha = torch.mul(result, x_j).sum(dim=-1)
