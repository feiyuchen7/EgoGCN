...
def data_load(dataset, has_v=True, has_a=True, has_t=True):
    ...
    if dataset == 'movielens':
        num_user = 55485
        num_item = 5986
        v_feat = np.load(dir_str+'/feat_v.npy', allow_pickle=True) if has_v else None
        a_feat = np.load(dir_str+'/feat_a.npy', allow_pickle=True) if has_a else None
        t_feat = np.load(dir_str+'/feat_t.npy', allow_pickle=True) if has_t else None
        ...
        if has_v:
            ...
            v_feat = v_feat.float().cuda()
        else:
            v_feat = None

        if has_a:
            a_feat = torch.load(dir_str+'/feat_a.pt')
            a_feat = a_feat.float().cuda() 
        else:
            a_feat = None
