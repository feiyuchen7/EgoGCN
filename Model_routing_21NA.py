...
class CGCN(torch.nn.Module):
    def __init__(self, num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm, alpha_threshold, reattn, central, is_word=False):
        ...
        #self.preference2 = nn.Parameter(nn.init.xavier_normal_(torch.rand((num_user, dim_C))))   
        #self.preference3 = nn.Parameter(nn.init.xavier_normal_(torch.rand((num_user, dim_C))))   
    ...
    def forward(self, preference, features, features2, features3, edge_index):
        if self.has_norm:
            preference = F.normalize(preference)     #normalize preference   (36656,64)
        ...
        #----------------------------------------
        edge_all = torch.cat((edge_index, edge_index[[1,0]]), dim=1)
        x =  torch.cat((preference, features, features2, features3), dim=0)
        for i in range(self.num_routing):       #routing
            x_hat_1 = self.conv_embed_2(x, edge_all)
            x = x + x_hat_1
            if self.has_norm:
                x = F.normalize(x)
        #----------------------------------------
        ...
        out_x = torch.cat((x[:self.num_user],(x[self.num_user:self.num_user+self.num_item] + x[self.num_user+self.num_item:self.num_user+self.num_item*2] + x[self.num_user+self.num_item*2:])/3),dim=0)
        out_x_hat_1 = torch.cat((x_hat_1[:self.num_user],(x_hat_1[self.num_user:self.num_user+self.num_item] + x_hat_1[self.num_user+self.num_item:self.num_user+self.num_item*2] + x_hat_1[self.num_user+self.num_item*2:])/3),dim=0)
        return out_x + out_x_hat_1, self.conv_embed_2.alpha.view(-1, 1)  
...
class Net(torch.nn.Module):
    def __init__(self, num_user, num_item, edge_index, user_item_dict, reg_weight, 
        ...
        self.mm_edge = mm_edge
        self.pruning = pruning

        self.gcn = CGCN(num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm, self.alpha_threshold, self.reattn, self.central)

        self.model_specific_conf = nn.Parameter(nn.init.xavier_normal_(torch.rand((num_user+num_item, 3)))) 
    ...
    def edgeNA(self, edge_index):

        edge,i = torch.sort(edge_index,-1)
        edge[0] = edge[0][i[-1]]

        m2 = torch.arange(self.num_item + self.num_user, self.num_item + self.num_user + self.num_item)

        m3 = torch.arange(self.num_item + self.num_user + self.num_item, self.num_item + self.num_user + self.num_item + self.num_item)
    ...
    def forward(self):
        ...
        mm_edge = self.mm_edge
        
        if self.central == 'central_user':
            mm_edge = mm_edge[[1,0]]
        features_v = F.leaky_relu(self.MLPv(self.v_feat))   
        ...
        '''if self.v_feat is not None:
            num_modal += 1
            v_rep, weight_v = self.v_gcn(self.preferencev, features_v, self.preferencea, features_a, self.preferencet, features_t, edge_index)
            weight = weight_v   #[1128730, 1]
            content_rep = v_rep #[112741, 64]

        if self.a_feat is not None:
            num_modal += 1
            a_rep, weight_a = self.a_gcn(self.preferencea, features_a, self.preferencev, features_v, self.preferencet, features_t, edge_index)
            if weight is  None:
                weight = weight_a  
                content_rep = a_rep
            else:
        ...
        if self.t_feat is not None:
            num_modal += 1
            t_rep, weight_t = self.t_gcn(self.preferencet, features_t, self.preferencea, features_a, self.preferencev, features_v, edge_index)
            if weight is None:
                weight = weight_t   
                conetent_rep = t_rep
            else:
        ...
        content_rep, weight = self.gcn(self.preference, features_v, features_a, features_t, mm_edge)
    ...
    def loss(self, user_tensor, item_tensor):
        ...
        reg_content_loss = reg_content_loss + (self.preference[user_tensor]**2).mean()
